...
echo "Getting metadata for total of $(echo $biosamples_accession_ids | wc -w) biosamples..."
queryIds=$(echo $biosamples_accession_ids | sed 's/ / OR /g')
esearch -db biosample -query "$queryIds" | efetch -format docsum -mode json > "$output_filepath"
